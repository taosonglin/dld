...
var render = {
  ...
  renderImage: function renderImage(callback) {
    ...
    setStyle(image, extend({
      ...
      marginLeft: !this.isPrev && this.isView ? 0: imageData.left,
...
var handlers = {
  ...
  load: function load() {
    ...
    image.style.cssText = 'width:0;' + 'height:0;' + ('margin-left:0px;') + ('margin-top:' + viewerData.height / 2 + 'px;') + 'max-width:none!important;' + 'visibility:visible;';
...
var methods = {
  ...
  view: function view(index) {
    ...
    this.isPrev = this.index > index;  //是不是上一张
    this.isView = true;
    this.viewed = false;
    this.index = index;
    this.imageData = null;
    ...
    // empty(canvas);
    var currImgs = canvas.querySelectorAll('img');
    if (currImgs.length) {
        if (currImgs.length > 2) {
            currImgs.forEach((v, k) => {
                if(v.getAttribute('index')!=this.index) {
                    canvas.removeChild(v);
                }
            });
        } else {
          image.classList.add('viewer-transition-width');
          currImgs[0].classList.add('viewer-transition-width');
          currImgs[0].style.width = 0;
          setTimeout(()=>{
            try {
                canvas.removeChild(currImgs[0]);
            } catch (error) {
              canvas.querySelectorAll('img').forEach((v)=>{
                if(v.getAttribute('index')!=this.index) {
                    canvas.removeChild(v);
                }
              })
            ...
            try {
                image.classList.remove('viewer-transition-width');
            } catch (error) {}
          }, 300);
        }
    } else {
    ...
    if (this.isPrev && currImgs.length == 1) {
      currImgs[0].style.marginLeft = 0;
      canvas.insertBefore(image, currImgs[0]);
    } else {
      ...
      if (options.toolbar) {
        ...
        each(custom ? options.toolbar : BUTTONS, function (value, index) {
          ...
          if (!isFunction(click)) {
            ...
            if (name === 'prev') {
              item.setAttribute('title', '上一张');
            } else if (name === 'next') {
              item.setAttribute('title', '下一张');
            } else if (name === 'play') {
              item.setAttribute('title', '全屏');
            } else if (name === 'zoom-in') {
              item.setAttribute('title', '放大');
            } else if (name === 'zoom-out') {
              item.setAttribute('title', '缩小');
            } else if (name === 'one-to-one') {
              item.setAttribute('title', '原始比例');
            } else if (name === 'reset') {
              item.setAttribute('title', '重置');
            } else if (name === 'rotate-left') {
              item.setAttribute('title', '逆时针旋转');
            } else if (name === 'rotate-right') {
              item.setAttribute('title', '顺时针旋转');
            } else if (name === 'flip-horizontal') {
              item.setAttribute('title', '水平翻转');
            } else if (name === 'flip-vertical') {
              item.setAttribute('title', '垂直翻转');
            }
          }
